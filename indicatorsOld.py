...
def calculate_ichimoku_cloud(df, symbols):
    ...
    for symbol in symbols:
        ...
        if high_col in df.columns and low_col in df.columns:
            ...
            df[f'{symbol}_tenkan_sen'] = (df[high_col].rolling(window=tenkan_sen_window).max() + df[low_col].rolling(
                window=tenkan_sen_window).min()) / 2
            df[f'{symbol}_kijun_sen'] = (df[high_col].rolling(window=kijun_sen_window).max() + df[low_col].rolling(
                window=kijun_sen_window).min()) / 2
            df[f'{symbol}_senkou_span_a'] = ((df[f'{symbol}_tenkan_sen'] + df[f'{symbol}_kijun_sen']) / 2).shift(
                kijun_sen_window)
            df[f'{symbol}_senkou_span_b'] = ((df[high_col].rolling(window=senkou_span_b_window).max() + df[
                low_col].rolling(window=senkou_span_b_window).min()) / 2).shift(kijun_sen_window)
...
def calculate_cci(df, symbols, window=20, constant=0.015):
    ...
    for symbol in symbols:
        ...
        if high_col in df.columns and low_col in df.columns and close_col in df.columns:
            ...
            df[f'{symbol}_mean_dev'] = df[f'{symbol}_tp'].rolling(window=window).apply(
...
def calculate_williams_r(df, symbols, period=14):
    ...
    for symbol in symbols:
        ...
        if high_col in df.columns and low_col in df.columns and close_col in df.columns:
            ...
            df[f'{symbol}_williams_r'] = -100 * (df[f'{symbol}_highest_high'] - df[close_col]) / (
                        df[f'{symbol}_highest_high'] - df[f'{symbol}_lowest_low'])
        else:
...
def calculate_chaikin_money_flow(df, symbols, period=20):
    ...
    for symbol in symbols:
        ...
        if high_col in df.columns and low_col in df.columns and close_col in df.columns and volume_col in df.columns:
            ...
            # CMF Calculation
            money_flow_multiplier = ((df[close_col] - df[low_col]) - (df[high_col] - df[close_col])) / (
                        df[high_col] - df[low_col])
            money_flow_volume = money_flow_multiplier * df[volume_col]
            df[f'{symbol}_cmf'] = money_flow_volume.rolling(window=period).sum() / df[volume_col].rolling(
                window=period).sum()
        else:
...
def calculate_volume_oscillator(df, symbols, short_period=12, long_period=26):
    for symbol in symbols:
        ...
        if volume_col in df.columns:
            df[f'{symbol}_vo'] = df[volume_col].rolling(window=short_period).mean() - df[volume_col].rolling(
...
def identify_candlestick_patterns(df, symbols):
    for symbol in symbols:
        ...
        if all(col in df.columns for col in [open_col, high_col, low_col, close_col]):
            ...
            df[f'{symbol}_doji'] = np.where(
                np.abs(df[close_col] - df[open_col]) <= ((df[high_col] - df[low_col]) * 0.1), 1, 0)
            df[f'{symbol}_hammer'] = np.where(
                (np.abs(df[close_col] - df[open_col]) <= (df[high_col] - df[low_col]) * 0.35) & (
                            (df[low_col] - np.minimum(df[close_col], df[open_col])) >= np.abs(
                        df[close_col] - df[open_col]) * 2), 1, 0)
        else:
...
def calculate_frama(df, symbol, window=16, fc=1, sc=198):
    ...
    if close_col in df.columns:
        ...
        for i in range(window, len(df)):
            df[f'{symbol}_frama'].iloc[i] = df['alpha'].iloc[i] * df[close_col].iloc[i] + (1 - df['alpha'].iloc[i]) * \
                                            df[f'{symbol}_frama'].iloc[i - 1]
...
def calculate_vidya(df, symbol, short_window=12, long_window=26, alpha=2):
    ...
    if close_col in df.columns:
        ...
        df['cmo'] = 100 * (
                    up.rolling(window=short_window).sum() - down.rolling(window=short_window).sum()) / short_window
...
def calculate_kama(df, symbol, er_period=10, fast_sc=2, slow_sc=30):
    ...
    if close_col in df.columns:
        ...
        for i in range(er_period, len(df)):
            df[f'{symbol}_kama'].iloc[i] = df[f'{symbol}_kama'].iloc[i - 1] + sc.iloc[i] * (
                        df[close_col].iloc[i] - df[f'{symbol}_kama'].iloc[i - 1])
...
def calculate_schaff_trend_cycle(df, symbols, macd_fast=23, macd_slow=50, stoch_period=10):
    for symbol in symbols:
        ...
        if close_col in df.columns:
            df[close_col] = pd.to_numeric(df[close_col], errors='coerce')
            macd_line = df[close_col].ewm(span=macd_fast, adjust=False).mean() - df[close_col].ewm(span=macd_slow,
                                                                                                   adjust=False).mean()
            macd_signal = macd_line.ewm(span=stoch_period, adjust=False).mean()
            stoch = 100 * (macd_line - macd_signal) / (
                        macd_line.rolling(window=stoch_period).max() - macd_signal.rolling(window=stoch_period).min())
...
def fetch_portfolio_data_binance_with_indicators(symbols, interval='1d', limit=30):
    ...
    for symbol in symbols:
        df = app.fetch_binance_data(symbol, interval, limit)
        price_data = df['close'].tolist()  # Or any other relevant price column
        df.columns = [f"{symbol}_{col}" for col in df.columns]

        # Apply all indicators
        df = calculate_rsi(df, [symbol])
        df = calculate_macd(df, [symbol])
        df = calculate_bollinger_bands(df, [symbol])
        df = calculate_ichimoku_cloud(df, [symbol])
        df = calculate_average_true_range(df, [symbol])
        df = calculate_stochastic_oscillator(df, [symbol])
        df = calculate_williams_r(df, [symbol])
        df = calculate_fibonacci_retracements(df, [symbol])
        df = calculate_mcclellan_oscillator(df, 'advance_col', 'decline_col')
        df = calculate_linear_regression_channels(df, [symbol])
        df = calculate_sma(df, [symbol])
        df = calculate_mama(df, symbol)
        df = calculate_garch(df)
        df = calculate_parabolic_sar(df, [symbol])
        df = calculate_volume_oscillator(df, [symbol])
        df = calculate_cci(df, [symbol])
        df = calculate_dmi_adx(df, [symbol])
        df = calculate_rate_of_change(df, [symbol])
        df = calculate_on_balance_volume(df, [symbol])
        df = calculate_accumulation_distribution_line(df, [symbol])
        df = calculate_money_flow_index(df, [symbol])
        df = calculate_chaikin_money_flow(df, [symbol])
        df = calculate_relative_strength_index(df, [symbol])
        df = calculate_pivot_points(df, [symbol])
        df = calculate_keltner_channels(df, [symbol])
        df = find_elliott_wave_peaks(df, [symbol])
        df = calculate_z_score(df, [symbol])
        df = calculate_hurst_exponent(df, [symbol])
        df = calculate_standard_deviation(df, [symbol])
        df = calculate_donchian_channels(df, [symbol])
        df = calculate_advance_decline_line(df, [symbol])
        df = calculate_mcclellan_summation_index(df)
        df = calculate_high_low_index(df, [symbol])
        df = calculate_price_channels(df, [symbol])
        df = identify_candlestick_patterns(df, [symbol])
        df = calculate_renko_bricks(df, [symbol], 5)
        df = calculate_heikin_ashi(df, [symbol])
        df = calculate_simple_moving_average(df, [symbol])
        df = calculate_arms_index(df, [symbol])
        df = calculate_frama(df, [symbol])
        df = calculate_vidya(df, [symbol])
        df = calculate_kama(df, [symbol])
        df = calculate_schaff_trend_cycle(df, [symbol])
        df = calculate_cycle_identifier(df, [symbol])
        df = calculate_detrended_price_oscillator(df, [symbol])
        df = calculate_put_call_ratio(df, [symbol])
        df = calculate_bid_ask_spread(df, [symbol])
        df = calculate_vw_bid_ask_spread(df, [symbol])
        df = calculate_exponential_smoothing(df, [symbol])
        df = calculate_arima(df, [symbol])
        df = calculate_sharpe_ratio(df, [symbol])
        df = calculate_sortino_ratio(df, [symbol])
